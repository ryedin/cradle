...
Database.prototype.get = function (id, rev, options) {
...
//
Database.prototype.put = function (id, doc, options, callback) {
    var args = new (Args)(arguments),
        cache = this.cache;
...
//
Database.prototype.post = function (doc, options, callback) {
    var args = new (Args)(arguments),
        cache = this.cache;
...
Database.prototype.save = function (/* [id], [rev], doc | [doc, ...] */) {
    ...
    if (Array.isArray(args.first)) {
        doc = args.first;
        if (args.first !== args.last) { options = args.last; }
    } else {
        if ((typeof array[1] === "string" && arguments.length == 5) || arguments.length == 4) {
            options = array.pop();
        }
        doc = array.pop(),
        id  = array.shift(),
    ...
    this._save(id, rev, doc, options, args.callback);
};

Database.prototype._save = function (id, rev, doc, options, callback) {
    var args = new (Args)(arguments),
        document = {}, 
        that = this;
    ...
    if (typeof options === "function") {
        ...
        options = cradle.merge({}, this.connection.options, options || {});
    }
    ...
    if (Array.isArray(doc)) {
        ...
        if (id) {
            ...
            if (document._rev) {
                this.put(id, document, options, callback);
            } else if (this.cache.has(id)) {
                ...
                this.put(id, document, options, function (e, res) {
        ...
        // POST a single document, without an id (Create)
            ...
            this.post(doc, options, callback);
        }
    }
};
